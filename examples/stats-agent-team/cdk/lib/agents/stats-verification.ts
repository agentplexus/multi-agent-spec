...
export interface StatsVerificationAgentProps {
  readonly foundationModel?: string;
}

export class StatsVerificationAgent extends Construct {
  public readonly agent: bedrock.CfnAgent;
  public readonly agentAlias: bedrock.CfnAgentAlias;

  constructor(scope: Construct, id: string, props?: StatsVerificationAgentProps) {
    super(scope, id);
    ...
    // Agent instruction
    const instruction = `You are a statistics verification specialist. Your role is to validate that extracted statistics actually exist in their claimed sources.
...
- Paywall blocking access to content`;
    ...
    // Output the agent ID
    new cdk.CfnOutput(this, 'StatsVerificationAgentId', {
      value: this.agent.attrAgentId,
      description: 'Agent ID for stats-verification',
    });
  }
}
